...
test('popular tags reflect mocked data', async ({ page }) => {
...
test('delete article', async ({page, request}) => {
  const articleResponse = await request.post('https://conduit-api.bondaracademy.com/api/articles/', {
    data: {
      "article":{"title":"ARticle title","description":"description article","body":"somet stuff","tagList":[]}
    },
  })
...
test('create article', async({page, request})=> {
  ...
  const articleResBody = await articleResponse.json()
  ...
  const response = await request.post('https://conduit-api.bondaracademy.com/api/users/login', {
    data: {
      "user": {email: "porkytester@test.com", password: "porkytester"}
    }
  })
